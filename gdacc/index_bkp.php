...
$RP = 3911;
// 
$update_limpia_guardia = 'update gda_persona, gda_grupo_persona, gda_grupo, gda_role 
set gda_persona.guardia = 0 
...
if (!(empty($_GET))) {
  ...
  //print("<img src='img/gda_logo_cuadrado.png' witdh='100px' height='100px'/></br></br>");


  $query_asigna = "SELECT gda_persona.Nombre, gda_persona.Apellido FROM gda_persona, gda_grupo_persona, gda_grupo, gda_role
  WHERE gda_grupo_persona.IDRole = gda_role.IDRole
  AND gda_grupo_persona.IdGrupo = gda_grupo.IDGrupo
  AND gda_grupo_persona.IDPersona = gda_persona.Legajo
  AND gda_persona.Guardia = 1
  ...
  $result = mysqli_query($conexion, $query_asigna);
  while ($res = mysqli_fetch_assoc($result)) {
    print("Actualmente la guardia esta derivada a " . $res['Nombre'] . " " . $res['Apellido']);
  }
printf("<h3>Para que te activen la Guardia deben llamarte al 0343481".$RP."</h3>");
